...
class MongoBackend {
  ...
  async connect() {
    ...
    this.collection = this.client.db("maxcoin")
        .collection("values");
    return this.client;
  }
  ...
  async max() {
    ...
    if (client.isConnected()) console.info("Successfully connected to MongoDB");
    else throw new Error("Connecting to MongoDB failed");
